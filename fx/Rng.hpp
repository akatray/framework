...
namespace fx::rng
{
	...
	// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	template<class T> auto rbuf ( const u64 _Size, T* _Buffer, const T _Lower, const T _Upper, const u32 _Seed = 0 )
	{
		auto Seed = u32(0);
		if(_Seed == 0) Seed = std::random_device()();
		else Seed = _Seed;
	...
	// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	template<class T> auto rbuf_nrm ( const u64 _Size, T* _Buffer, const T _Mean = T(0), const T _Sigma = T(1), const u32 _Seed = 0 )
	{
		auto Seed = u32(0);
		if(_Seed == 0) Seed = std::random_device()();
		else Seed = _Seed;
